...
"""
업데이트된 쌍바닥 분석 코드
- 최근 최저가가 이전 최저가와 쌍바닥을 이루고
- 최근 최저가 이후 3일간 최저가 근처에서 횡보하는 조건 추가
"""
...
class UpdatedDoubleBottomAnalyzer:
    """업데이트된 쌍바닥 패턴 분석 클래스"""
    ...
    def check_sideways_below_neckline(self, data, peak_idx, days=5, tolerance_pct=0.03):
        """
        넥라인 밑에서 횡보 여부 확인
        ...
        Returns:
            dict: 횡보 분석 결과
        """
        if peak_idx + days >= len(data):
            return {'is_sideways': False, 'sideways_days': 0, 'max_price': 0, 'min_price': 0}
        
        # 넥라인 이후 N일간 데이터
        after_peak_data = data.iloc[peak_idx:peak_idx + days + 1]
        peak_price = data.iloc[peak_idx]['Close']
        
        # 넥라인 이후 최고가와 최저가
        max_price = after_peak_data['Close'].max()
        min_price = after_peak_data['Close'].min()
        
        # 넥라인 대비 변동폭이 허용 범위 내인지 확인
        max_deviation = max(abs(max_price - peak_price), abs(min_price - peak_price))
        deviation_pct = max_deviation / peak_price
        
        # 넥라인 밑에서 횡보하는지 확인 (넥라인보다 낮은 가격대에서 횡보)
        is_below_neckline = max_price < peak_price * 1.01  # 넥라인보다 1% 이내
        is_sideways = deviation_pct <= tolerance_pct and is_below_neckline
        
        # 실제 횡보 일수 계산
        sideways_days = 0
        if is_sideways:
            for i in range(len(after_peak_data)):
                current_price = after_peak_data.iloc[i]['Close']
                if abs(current_price - peak_price) / peak_price <= tolerance_pct:
                    sideways_days += 1
                else:
                    break
        ...
        return {
            ...
            'is_below_neckline': is_below_neckline
        }
    
    def validate_double_bottom_pattern(self, data, first_bottom_idx, second_bottom_idx, 
                                     first_bottom_price, second_bottom_price, peak_idx, peak_price, lookback_days=60):
        """
        쌍바닥 패턴 유효성 검증 (업데이트된 버전 - 넥라인 밑 횡보 포함)
        ...
        # 4. 넥라인 밑에서 횡보 확인
        neckline_sideways = self.check_sideways_below_neckline(data, peak_idx, days=5, tolerance_pct=0.03)
        ...
        validation_result = {
            ...
            'neckline_sideways': neckline_sideways,
        ...
        # 넥라인 밑 횡보 검증 (15점)
        if not neckline_sideways['is_sideways']:
            validation_result['issues'].append(f"넥라인 밑에서 횡보하지 않음")
            score -= 15
        else:
            # 넥라인 밑 횡보하면 보너스 점수
            score += 10
            # 횡보 일수가 길수록 더 높은 점수
            if neckline_sideways['sideways_days'] >= 3:
                score += 5
        ...
        for i, b1_idx in enumerate(minima_indices[:-1]):  # 마지막 바닥은 제외
            ...
            for b2_idx in b2_candidates:
                ...
                validation = self.validate_double_bottom_pattern(
                    df_recent, b1_idx, b2_idx, b1_price, b2_price, peak_idx, peak_price, 60)
    ...
    def print_valid_results(self):
        ...
        for i, result in enumerate(valid_results, 1):
            ...
            # 넥라인 밑 횡보 정보 출력
            if validation.get('neckline_sideways'):
                neckline_info = validation['neckline_sideways']
                print(f"    📊 넥라인 밑 횡보: {'✅ 횡보' if neckline_info['is_sideways'] else '❌ 횡보하지 않음'}")
                if neckline_info['is_sideways']:
                    print(f"    📊 넥라인 밑 횡보 일수: {neckline_info['sideways_days']}일")
    
    def save_valid_results_to_csv(self, filename='updated_valid_double_bottom_results.csv'):
        ...
        for result in valid_results:
            ...
            results_data.append({
                ...
                '넥라인밑횡보': '✅ 횡보' if validation.get('neckline_sideways', {}).get('is_sideways', False) else '❌ 횡보하지 않음',
                '넥라인밑횡보일수': validation.get('neckline_sideways', {}).get('sideways_days', 0),
        ...
        print(f"✅ 업데이트된 유효한 쌍바닥 패턴 결과가 '{filename}' 파일로 저장되었습니다.")
        print(f"📊 총 {len(valid_results)}개 종목이 유효한 쌍바닥 패턴으로 확인되었습니다.")

def main():
    """메인 실행 함수"""
    print("🚀 업데이트된 쌍바닥 분석 시작")
    print("="*60)
    print("📋 새로운 조건:")
    print("   • 최근 최저가가 이전 최저가와 쌍바닥을 이루고")
    print("   • 최근 최저가 이후 3일간 최저가 근처에서 횡보")
    print("   • 넥라인 밑에서 5일간 횡보 패턴 확인")
    print("="*60)
    
    # 분석기 초기화
    analyzer = UpdatedDoubleBottomAnalyzer('코스피6개월종가_20250908_035947.csv')
